...
class Wide_ResNet(nn.Module):
    def __init__(self, config):
        ...
        assert ((config.depth-4)%6 ==0), 'Wide-resnet depth should be 6n+4'
        n = (config.depth-4)/6
        ...
        self.linear = nn.Linear(nStages[3], config.num_classes)
    ...
    def activations(self, x):
        ...
        out = self.layer2(out)
        out_activations["layer2"] = out

        out = self.layer3(out)
        out_activations["layer3"] = out
